...
const ASVSListFilter: React.FC<Props> = ({
  chapters,
  setChapterCheck,
  setLevelCheck,
  ...
  return (
    <div className="rounded-lg h-full shadow p-2 bg-gray-50 font-normal border-gray-400 border overflow-y-scroll">
      ...
      <ul className="flex w-full flex-wrap gap-1">
        <li>
          ...
          <label
            ...
            className="inline-flex w-full p-1 text-gray-900 bg-white border border-gray-200 rounded-lg cursor-pointer peer-checked:border-blue-600 peer-checked:text-blue-600 hover:text-gray-600 hover:bg-gray-100"
          >
            Incomplete
          </label>
        </li>
        ...
        <li>
          <div key="level-1-checkbox" className="pl-3">
            ...
            <label htmlFor="level-1-checkbox">
              <span className="bg-green-200 text-green-900 text-xs font-medium m-1 px-1.5 py-0.5 rounded">
                Level 1
              </span>
            </label>
          </div>
        ...
        <li>
          <div key="level-2-checkbox" className="pl-3">
            ...
            <label htmlFor="level-2-checkbox">
              <span className="bg-yellow-200 text-yellow-900 text-xs font-medium m-1 px-1.5 py-0.5 rounded">
                Level 2
              </span>
            </label>
          </div>
        ...
        <li>
          <div key="level-3-checkbox" className="pl-3">
            ...
            <label htmlFor="level-3-checkbox">
              <span className="bg-red-200 text-red-900 text-xs font-medium m-1 px-1.5 py-0.5 rounded">
                Level 3
              </span>
            </label>
          </div>
      ...
      <ul className="pt-2 mt-2 border-t border-gray-400">
        ...
        {chapters.map((chapter: ASVSChapter, index: number) => (
          <ASVSChapterFilterItem
            key={chapter.name}
            chapterId={index + 1}
            name={chapter.name}
            checked={chapter.checked}
            toggleFunction={setChapterCheck}
          ></ASVSChapterFilterItem>
        ))}
      </ul>
    </div>
